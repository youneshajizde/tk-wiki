...
export default function KingdomWeaponsPage() {
  ...
  const kingdomWeapons = weapons[kingdomId] ?? [];
  ...
  const filteredWeapons = useMemo(() => {
    ...
    if (!query) {
      return kingdomWeapons;
    }

    return kingdomWeapons.filter((weapon) =>
      `${weapon.name} ${weapon.title} ${weapon.description}`
        .toLowerCase()
        .includes(query)
    );
  }, [search, kingdomWeapons]);
    ...
    <main className="min-h-screen bg-[#090807] text-stone-100">
      <div className="mx-auto max-w-7xl px-6 py-16 lg:px-10">
        ...
        <div className="flex gap-3 text-[9px] uppercase tracking-[0.25em] text-stone-700">
          ...
          <Link href="/weapons" className="hover:text-amber-400">
            Weapons
          </Link>
        ...
        <div className="mt-20 flex flex-col gap-10 md:flex-row md:items-center">
          ...
          <div>
            ...
            <p className="mt-4 max-w-xl text-sm leading-7 text-stone-500">
              Legendary weapons and artifacts belonging to the history of{" "}
              {kingdom.name}.
            </p>
          </div>
        </div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          ...
          {filteredWeapons.map((weapon) => (
            <Link
              key={weapon.id}
              href={`/weapons/${kingdomId}/${weapon.id}`}
